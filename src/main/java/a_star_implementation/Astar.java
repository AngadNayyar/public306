...
public class Astar {
	...
	private DefaultDirectedWeightedGraph<TaskNode, DefaultEdge> graph = new DefaultDirectedWeightedGraph<TaskNode, DefaultEdge>(
			...
			public Astar(DefaultDirectedWeightedGraph<TaskNode, DefaultEdge> graph, Options options,
					VisualisationGraph visualGraphObj) {
				this.graph = graph;
			...
			public boolean function(Path temp, double pathWeight) {
				Iterator itr = openQueue.iterator();
				...
				while(itr.hasNext()) {
					...
					if (schedule.pathWeight == pathWeight && (schedule.state.getPath().size() == temp.getPath().size())){
				...
				for (int k = 0; k < similarSchedules.size(); k++){
					...
					if (tempProcSet.containsAll(currentSet) && currentSet.containsAll(tempProcSet)){
				...
				/*
				 * 
				 * ArrayList<TaskNode> path = schedule.state.getPath(); for (TaskNode node:
				 * path) { exists = false; for (TaskNode tempNode: temp.getPath()) { if
				 * (node.name.equals(tempNode.name)) { if(node.startTime !=
				 * tempNode.startTime) { // node doesn't have same start time return false;
				 * } else { exists = true; } } } // node doesnt exist in both if (!exists){
				...
				public boolean isComplete(StateWeights stateWeight) {
					...
					if (all.isEmpty()) {
						/*
						 * for (StateWeights s: closedQueue){ System.out.print(
						 * "\n Closed path, weight: " + s.pathWeight + " "); for (Node n:
						 * s.state.getPath()){ System.out.print(n.name); System.out.print(
						 * " " + n.allocProc); } } for (StateWeights s: openQueue){
